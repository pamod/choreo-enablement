...
service / on new http:Listener(9090) {
    ...
    resource function post contacts(@http:Payload ContactInput contactsInput) returns ContactsOutput|error? {
...
function transform(ContactInput contactInput) returns ContactsOutput => {
    numberOfContacts: contactInput.totalSize,
    contacts: from var recordsItem in contactInput.records
        select {
